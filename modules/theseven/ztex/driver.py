...
class ZtexDevice(object):
  ...
  def __init__(self, proxy, serial, takeover, firmware):
    ...
    descriptor = self.handle.controlMsg(0xc0, 0x22, 40, 0, 0, 100)
    ...
    descriptor = self.handle.controlMsg(0xc0, 0x82, 64, 0, 0, 100)
    ...
    unconfigured, checksum, bytestransferred, initb, result, bitswap = struct.unpack("<BBIBBB", self.handle.controlMsg(0xc0, 0x30, 9, 0, 0, 100))
    if unconfigured:
      ...
      if self.hs_supported:
        ep, interface = struct.unpack("<BB", self.handle.controlMsg(0xc0, 0x33, 2, 0, 0, 100))
  ...
  def read_nonces(self):
    with self.lock:
      data = self.handle.controlMsg(0xc0, 0x81, 12 * self.num_nonces, 0, 0, 100)
    nonces = []
